...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe: pd.DataFrame) ->bool:
        try:
            numerical_column_list = self._schema_config["numerical_columns"]
            num_col_dfs = dataframe.select_dtypes(include = ['int64', 'float64','int32','float32'])
            if (len(num_col_dfs.columns) != len(numerical_column_list)):
                return False 
            for column in numerical_column_list:
                if column not in num_col_dfs.columns:
                    return False
